...
void getgatedata(vector<pair<int, int>> adj[], int V, int e, int gate_type[])
{
    ...
    for (int u = 0; u < V; u++)
    {
        for (auto it1 = adj[u].begin(); it1 != adj[u].end(); it1++)
        {
            ...
            if (v == i_gatedata[0])
            { // If node connected to node u is the gate connected to the previous wire
            ...
            if (u == i_gatedata[1])
            {                           // u is the gate that was connected to the output of e that we had before here
        ...
        if (pi == 1)
        { // e is a primary input
            i_gatedata[2] = -1;
            i_gatedata[3] = -1;
        }
        if (po == 1)
        { // e is a primary output
            i_gatedata[4] = -1;
            i_gatedata[5] = -1;
        }
        /*
...
int main()
{
    ...
    for (int i = 0; i < V; i++)
    {
        ...
        cout << "Node " << i << " is of type " << gate_type[i] << endl;
    }
    ...
    char dcubes[8][12][3] = {
        ...
        {{'0', '0', '0'}, {'1', '1', '1'}, {'N', 'N', 'N'}, {'N', 'N', 'N'}, {'0', '0', 'E'}, {'1', '1', 'D'}, {'N', 'N', 'N'}, {'N', 'N', 'N'}, {'D', 'D', 'D'}, {'E', 'E', 'E'}, {'N', 'N', 'N'}, {'N', 'N', 'N'}}}; // 0 to 3 are no errors, 4 to 7 are error at output, 8 to 11 are error at input AND output
